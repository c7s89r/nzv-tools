...
def _pre():
    ...
    if _cfg.get("auto_update"):
        try:
            import requests
            _r = requests.get("https://raw.githubusercontent.com/c7s89r/nzv-tools/main/core/config.json", timeout=5)
            if _r.status_code == 200:
                ...
                if parse_v(_rv) > parse_v(_cfg.get("version", "1.0.0")):
                    ...
                    _url = "https://github.com/c7s89r/nzv-tools/archive/refs/heads/main.zip"
...
def _nbot_ui():
    ...
    default_config = {
        "BOT_TOKEN": "",
        "MESSAGE_CONTENT": "@everyone | Server Nuked | https://github.com/c7s89r/nzv-tools",
    ...
    while True:
        ...
        cmd = get_inpt("nzv@nukebot:~#").strip().lower()
