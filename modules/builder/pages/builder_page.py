...
class BuildThread(QThread):
    ...
    def run(self):
        try:
            ...
            if success:
                self.finished_signal.emit(True, "NZV: build completed")
            else:
                self.finished_signal.emit(False, "NZV: build failed")
                
        except Exception as e:
            self.log(f"\nNZV: error - {str(e)}")
            self.finished_signal.emit(False, f"NZV: build error")
...
class BuilderPage(QWidget):
    ...
    def start_build(self):
        ...
        if not filename:
            self.box = CustomMessageBox("NZV", "error", "NZV: enter file name")
            self.box.show()
            return
